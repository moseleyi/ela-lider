...
<?php
	...
	switch($cms->a) {
		...
		case "list":
			echo '
			<div id="moduleWrap">
				<a href="'.$cms->get_link("news,add").'" class="greenButtonWide ">'.$cms->translate(63).'</a><div class="c"></div><br />
				<table class="table">
					<tr>
			...
			$i=1;
			$cms->executeQuery("SELECT * FROM cms_news WHERE lang='$cms->lang' ORDER BY date DESC",1);
			while($row = mysqli_fetch_assoc($cms->result1)) {
				echo '<tr>
						...
						<td class="body lvl0"><a href="'.$cms->get_link("news,status,".$row["id"]).'" class="link_'.($row["status"] == 1 ? 'show' : 'hide').' plink">&nbsp;</a></td>
					...
					// if galleries for each page
						...
						echo '
						<td class="body lvl0"><a href="'.$cms->get_link("news,edit,".$row["id"]).'" class="link_edit plink">&nbsp;</a></td>
		...
		case "edit":
			...
			/*** EDIT ONLY ***/
			if($cms->a == "edit"){
				$cms->executeQuery("SELECT * FROM cms_news WHERE id='$cms->id'",1);
			...
			if(isset($_POST["submit"]) || isset($_POST["save"])) {
				...
				elseif($cms->checkExistingNames("cms_news") == false && ($cms->urls == "current-noid" || $cms->urls == "full-noid")) {
                    ...
                    if(count($files["name"]) > 0) {
                        foreach($files["name"] as $i => $fname) {
                            ...
                            $n = $cms->getRandomName();
                            $cms->executeQuery("SELECT * FROM cms_image_dimensions cid INNER JOIN cms_modules cm ON cid.moduleId = cm.id WHERE cm.shortName='news' ORDER BY cid.id DESC",1);
                            while($row = mysqli_fetch_assoc($cms->result1)) {
                                ...
                                if($cms->makeThumbnail(array("name" => $fname, "tmp_name" => $files["tmp_name"][$i]),"_images_content/news/".$dir,$n, $row["width"], "width",$fit,$fill, $row["quality"]) == false) {
                                    $error++;
                                }
                            }
                            if($error == 0) {
                                ...
                                if($h > 1200) {
                                    $cms->makeThumbnail($file,"_images_content/news/_original", $n, 1200, "height", 0, 0, $row["quality"]);
                                    $cms->makeThumbnail($file,"_images_content/news/_cropped", $n, 1200, "height", 0, 0, $row["quality"]);
                                }
                                else {
                                    move_uploaded_file($files["tmp_name"][$i],"_images_content/news/_original/".$n.".".$ext);
                                    copy("_images_content/news/_original/".$n.".".$ext,"_images_content/news/_cropped/".$n.".".$ext);
                                }

                                $position = $cms->getCount("cms_news_files", "WHERE news_id='$cms->id'") + 1;
                                $fna = $n.'.'.$ext;
                                $cms->executeQuery("INSERT INTO cms_news_files (`id`, `file`, `news_id`, `position`, `alt`, `description`, `link`, `main`) VALUES ('','$fna','$cms->id','$position','','','', '0')",1);
                            }
                        }
                    }
					...
					/*** ADD ***/
					if($cms->a == "add") {
						$cms->executeQuery("INSERT INTO cms_news (`id`, `date`, `title`, `content`, `lang`,`galleryId`, `metaTitle`, `metaDesc`, `metaKeys`, `intName`, `status`, `file`) VALUES ('','$date','".$cms->esc($title)."','$content','$cms->lang','$gallery_id','".$cms->esc($meta_title)."','".$cms->esc($meta_desc)."','".$cms->esc($meta_keys)."', '$url', '0', '$cms->fn')",1);
					}

					/*** EDIT ***/
					else {
						$cms->executeQuery("UPDATE cms_news SET title='".$cms->esc($title)."',date='$date',intName='$url',content='$content',galleryId='$gallery_id',metaTitle='".$cms->esc($meta_title)."',metaDesc='".$cms->esc($meta_desc)."',metaKeys='".$cms->esc($meta_keys)."',file='$cms->fn' WHERE id='$cms->id'",1);
					}
					...
					if($cms->result1) {
						...
						if(!isset($_POST["save"])) {
							...
							header("Location:".$cms->get_link("news"));
						}
						else {
			...
			echo ' <form method="post" enctype="multipart/form-data" action="'.$cms->get_link("news,".$cms->a.''.($cms->a=="edit"?','.$cms->id:'')).'">
				...
				<div class="addColumn narrow left NB">
					...
					<div class="addColumn full clear NB">
						<div class="itemWrap ">
							...
							<ul id="image-sorting">';
								$cms->executeQuery("SELECT * FROM cms_news_files WHERE news_id='$cms->id' ORDER BY position ASC",1);
								while($row = mysqli_fetch_assoc($cms->result1)) {
									...
									echo '<li id="'.$row["id"].'" class="image-wrap">
											...
											<div class="imageTools">
												...
												'.($row["main"] == 0 ? '<a href="'.$cms->get_link("news,main,".$row["id"]).'" class="link_main_on plink blog_image_main">&nbsp;</a>' : '<a href="#" class="link_main_on2 plink blog_image_main">&nbsp;</a>').'
											</div>
											<div class="clear"></div>
										</li>';
								}
		...
		case "del":
			$cms->executeQuery("SELECT * FROM cms_news WHERE id='$cms->id'",1);
			$row = mysqli_fetch_assoc($cms->result1);
			$cms->executeQuery("DELETE FROM cms_news WHERE id='$cms->id'",1);
			if($cms->result1) {
		...
		case "status":
			...
			$status = $row["status"];
			$n = $status == 0 ? 1 : 0;
			$cms->executeQuery("UPDATE cms_news SET status='$n' WHERE id='$cms->id'",1);
			if($cms->result1) {
		...
		break;
		case "delimg":
			$cms->executeQuery("SELECT *,g.id AS id,gf.file AS file FROM cms_news_files gf LEFT JOIN cms_news g ON gf.news_id=g.id WHERE gf.id='$cms->id'",1);
			$row = mysqli_fetch_assoc($cms->result1);
			$file = $row["file"];
			$news_id = $row["id"];
			$cms->executeQuery("DELETE FROM cms_news_files WHERE id='$cms->id'",2);
			if($cms->result2) {
			...
			header("Location:".$cms->get_link("news,edit,$news_id#edit"));
		break;
		case "moveimg":
			$cms->executeQuery("SELECT * FROM cms_news WHERE id='$cms->id'",1);
			$row = mysqli_fetch_assoc($cms->result1);
			...
			for ($idx = 0; $idx < count($ids); $idx+=1) {
				$id = $ids[$idx];
				$ordinal = $idx;
				$cms->executeQuery("UPDATE cms_news_files SET position='$ordinal' WHERE id='$id'",1);
			}
			$cms->saveAction($title,"","news","moveimg");
		break;
		case "editd":
			$cms->executeQuery("SELECT * FROM cms_news_files WHERE id='$cms->id'",2);
			$row = mysqli_fetch_assoc($cms->result2);
			$news_id = $row["news_id"];
			$file = $row["file"];
			$cms->executeQuery("SELECT * FROM cms_news WHERE id='$post_id'",3);
			$row2 = mysqli_fetch_assoc($cms->result3);
			$title = $row2["title"];
			$alt = $_POST["alt"];
			$desc = $_POST["desc"];
			$link = $_POST["link"];
			$cms->executeQuery("UPDATE cms_news_files SET alt='".$cms->esc($alt)."', description='".$cms->esc($desc)."', link='".$cms->esc($link)."' WHERE id='$cms->id'",1);
			if($cms->result1) {
		...
		break;
		case "main":
			$cms->executeQuery("SELECT * FROM cms_news_files WHERE id='$cms->id'",1);
			$row = mysqli_fetch_assoc($cms->result1);
			$news_id = $row["news_id"];
			$file = $row["file"];
			$cms->executeQuery("SELECT * FROM cms_news WHERE id='$news_id'",2);
			$row2 = mysqli_fetch_assoc($cms->result2);
			$title = $row2["title"];
			$cms->executeQuery("UPDATE cms_news_files SET main='0' WHERE news_id='$news_id'",3);
			if($cms->result3) {
				$cms->executeQuery("UPDATE cms_news_files SET main='1' WHERE id='$cms->id'",4);
				if($cms->result4) {
